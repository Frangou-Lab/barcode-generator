...
{
	char *ret = strchr(g_alpgabet, (int)c);
	if (!ret)
		return (t_code)0x7f;
	return (t_code)(ret - g_alpgabet);
}
...
int BarCode::dectectCodeLen(char *str, int colNumber, char separator)
{
	SearchResult r = findColumn(str, colNumber, separator);
	if (r.position < 0)
		return false;
...
void BarNodeManager::addPage()
{
	...
	if (m_pages) // Realloc
		m_pages = (uint8_t **)realloc(m_pages, m_pagesCount);
	else // New
		m_pages = (uint8_t **)malloc(m_pagesCount);
	m_pages[m_pagesCount-1] = page;
}
...
{
	if (m_pos+sizeof(SimpleBarNode) >= getPageSize() || NULL == m_pages) // Realloc
		addPage();
	SimpleBarNode* node = (SimpleBarNode *)(m_pages[m_pagesCount-1]+m_pos);
	// Init node
...
{
	if (m_pos+sizeof(PagedPointerBarNode) >= getPageSize() || NULL == m_pages) // Realloc
		addPage();
	if (parent != NULL)
	{
		PagedPointerBarNode* typifiedParent = (PagedPointerBarNode*) parent;
		typifiedParent->m_pages[childIndex] = m_pagesCount - 1;
		typifiedParent->m_offsets[childIndex] = m_pos;
	}
	PagedPointerBarNode* node = (PagedPointerBarNode*) (m_pages[m_pagesCount - 1]+m_pos);
	memset(node->m_offsets, 0, CODE_SIZE*sizeof(uint32_t)+CODE_SIZE*sizeof(uint8_t));
